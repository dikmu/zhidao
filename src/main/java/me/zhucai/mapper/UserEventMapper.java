...
public interface UserEventMapper {
    ...
    @Select("select user_id userId,opt,opt_place optPlace,opt_time optTime,opt_how optHow from user_event t where t.user_id = #{userId} and t.opt_time >= to_timestamp(#{dateStartStr},'yyyy-mm-dd') and t.opt = #{opt} and t.opt_how != #{optHow}")
    List<UserEvent> countUserDailyOpt(@Param("userId") String userId, @Param("dateStartStr") String dateStartStr, @Param("opt") String opt, @Param("optHow") String optHow);
